...
function buildQrCodeUrl(content) {
    return `https://quickchart.io/qr?size=220&text=${encodeURIComponent(content)}`;
}
function buildMapsUrl(location, address) {
    const q = encodeURIComponent((address === null || address === void 0 ? void 0 : address.trim()) || location);
...
async function fetchAssetBuffer(req, rawUrl) {
    ...
    if (trimmed.startsWith("/uploads/")) {
        const filePath = path_1.default.join(process.cwd(), trimmed.replace(/^\//, ""));
...
async function convertWebpToJpg(buffer) {
    return (0, sharp_1.default)(buffer).jpeg({ quality: 88 }).toBuffer();
}
function isPngOrJpg(buffer) {
    if (buffer.length < 4)
        return false;
    const pngSig = buffer.slice(0, 8).toString("hex");
    if (pngSig === "89504e470d0a1a0a")
        return true;
    const jpgSig = buffer.slice(0, 2).toString("hex");
    return jpgSig === "ffd8";
}
async function ensurePdfImageBuffer(buffer, rawUrl) {
    if (isPngOrJpg(buffer))
        return buffer;
    try {
...
exports.invitationsRouter.get("/:token", async (req, res) => {
    var _a;
    try {
        ...
        await prisma_1.prisma.guestInvitation.update({
        ...
        const drinksEnabled = (_a = invitation.guest.event.drinksEnabled) !== null && _a !== void 0 ? _a : true;
        const drinks = drinksEnabled
            ? await prisma_1.prisma.drinkOption.findMany({
                where: { eventId: invitation.guest.eventId }
            })
            : [];
        const guestbookMessages = await prisma_1.prisma.guestBookMessage.findMany({
            where: { eventId: invitation.guest.eventId, isHidden: false },
            include: { guest: true },
            orderBy: { createdAt: "desc" },
            take: 50
        });
        const programItems = await prisma_1.prisma.eventProgramItem.findMany({
            where: { eventId: invitation.guest.eventId },
            orderBy: { order: "asc" }
        });
        const gifts = await prisma_1.prisma.giftItem.findMany({
            where: { eventId: invitation.guest.eventId },
            orderBy: { createdAt: "desc" }
        });
        ...
        res.json({
            ...
            guestbookMessages: guestbookMessages.map(item => {
                ...
                return ({
                    ...
                    guestName: (_b = (_a = item.guest) === null || _a === void 0 ? void 0 : _a.fullName) !== null && _b !== void 0 ? _b : "Invite anonyme"
                });
            })
        });
    }
...
exports.invitationsRouter.post("/:token/upload-media", async (req, res) => {
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
            return res.status(400).json({ message: "Token invalide." });
        ...
        const filename = `${Date.now()}-${crypto_1.default.randomBytes(4).toString("hex")}-${safeBase}.${parsed.ext}`;
        const uploadDir = path_1.default.join(process.cwd(), "uploads");
        await promises_1.default.mkdir(uploadDir, { recursive: true });
        await promises_1.default.writeFile(path_1.default.join(uploadDir, filename), parsed.buffer);
...
exports.invitationsRouter.post("/:token/memories", async (req, res) => {
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
            return res.status(400).json({ message: "Token invalide." });
        ...
        const memory = await prisma_1.prisma.eventMemory.create({
            data: {
...
exports.invitationsRouter.get("/:token/chat", async (req, res) => {
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
            return res.status(400).json({ message: "Token invalide." });
...
exports.invitationsRouter.post("/:token/chat", async (req, res) => {
    var _a;
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
...
async function setGuestStatusFromToken(token, status) {
    if (!isValidToken(token))
        return null;
    const invitation = await prisma_1.prisma.guestInvitation.findUnique({ where: { token } });
    if (!invitation)
        return null;
    const updatedGuest = await prisma_1.prisma.guest.update({
...
async function updateGuestRsvpData(guestId, payload) {
    const data = {};
...
// Confirmer presence
exports.invitationsRouter.post("/:token/confirm", async (req, res) => {
    try {
        const guest = await setGuestStatusFromToken(req.params.token, "CONFIRMED");
        if (!guest) {
            return res.status(404).json({ message: "Invitation introuvable." });
        }
        await updateGuestRsvpData(guest.id, req.body);
        res.json({ guest });
    }
    catch (err) {
        console.error(err);
...
// Annuler presence
exports.invitationsRouter.post("/:token/cancel", async (req, res) => {
    try {
...
exports.invitationsRouter.post("/:token/drinks", async (req, res) => {
    var _a;
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
        ...
        if (((_a = invitation.guest.event) === null || _a === void 0 ? void 0 : _a.drinksEnabled) === false) {
            return res.status(400).json({ message: "Le choix des boissons est desactive." });
        }
        await prisma_1.prisma.guestDrinkChoice.deleteMany({
            where: { guestId }
        });
        ...
        if (Array.isArray(choices) && choices.length > 0) {
            const normalizedChoices = choices
                ...
                quantity: Number.isInteger(choice.quantity) && choice.quantity > 0 && choice.quantity <= 20
                    ? choice.quantity
                    : 1
            }));
            const allowedDrinks = await prisma_1.prisma.drinkOption.findMany({
                where: { eventId: invitation.guest.eventId },
            ...
            const maxPerGuestMap = new Map(allowedDrinks.map(d => { var _a; return [d.id, (_a = d.maxPerGuest) !== null && _a !== void 0 ? _a : null]; }));
            await prisma_1.prisma.guestDrinkChoice.createMany({
...
exports.invitationsRouter.post("/:token/guestbook", async (req, res) => {
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
            return res.status(400).json({ message: "Token invalide." });
        ...
        const saved = await prisma_1.prisma.guestBookMessage.create({
            data: {
                message: message.trim(),
                guestId: invitation.guestId,
                eventId: invitation.guest.eventId,
                isHidden: Boolean(event === null || event === void 0 ? void 0 : event.guestbookRequiresApproval)
            }
        });
...
exports.invitationsRouter.get("/:token/pdf", async (req, res) => {
    try {
        const { token } = req.params;
        if (!isValidToken(token)) {
            return res.status(400).json({ message: "Token invalide." });
        ...
        const doc = new pdfkit_1.default({ size: "A4", margin: 46 });
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `attachment; filename="invitation-${invitationData.guest.fullName}.pdf"`);
        doc.pipe(res);
        ...
        async function drawCoverPage() {
            ...
            doc.text(invitationData.guest.event.dateTime.toLocaleString("fr-FR"), 46, pageH - 180, { width: contentW });
        ...
        function drawInvitationPage() {
            ...
            doc.fillColor(primary).font(fonts.bold).fontSize(12).text(`Cher(e) ${invitationData.guest.fullName},`, 64, greetingTop + 16);
            doc.font(fonts.regular).fontSize(11).fillColor(text).text(`Vous etes invite(e) a ${invitationData.guest.event.name}.`, 64, greetingTop + 38, { width: contentW - 36 });
        ...
        async function drawQrPage() {
            ...
            doc.font(fonts.regular).fontSize(11).fillColor(muted).text("Presentez ce code a l'entree.", 46, 92);
            const qrBox = 220;
        ...
        async function drawProgramPage() {
            ...
            doc.font(fonts.regular).fontSize(11).fillColor(muted).text("Deroulement de l'evenement", 46, 92);
            const programItems = await prisma_1.prisma.eventProgramItem.findMany({
            ...
            if (programItems.length > 0) {
                ...
                programItems.forEach((item, index) => {
                    ...
                    let blockHeight = 18;
                    if (item.description) {
                        const descHeight = doc.heightOfString(item.description, { width: textW });
                ...
                doc.font(fonts.regular).fontSize(11).fillColor("#374151").text(toPlainText(program), 46, 120, { width: contentW });
            }
            else {
                doc.font(fonts.regular).fontSize(11).fillColor("#6B7280").text("Le programme detaille sera communique par l'organisateur.", 46, 120, { width: contentW });
            }
            doc
                .font(fonts.italic)
                .fontSize(10)
